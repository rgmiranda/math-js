...
describe(Segment.name, () => {
    ...
    it('detects a point on a segment', () => {
        ...
        expect(s.contains(new Vector(0, 0))).toBe(true);
        expect(s.contains(new Vector(0.5, 0))).toBe(false);
    });
    ...
    const segmentsIntersections = [
        ...
        [new Segment(new Vector(1, 1), new Vector(-1, -1)), new Segment(new Vector(-1, 1), new Vector(-1, 0)), false],
    ];
    ...
    it('calculates midpoint', () => {
        const s = new Segment(new Vector(0, 0), new Vector(2, 1));
        expect(s.midpoint.x).toBe(1);
        expect(s.midpoint.y).toBe(0.5);
    });

    it('calculates lenght', () => {
        const s = new Segment(new Vector(1, 1), new Vector(5, 4));
        expect(s.length).toBe(5);
    });

    it('calculates direction', () => {
        const s = new Segment(new Vector(1, 0), new Vector(1, -1));
