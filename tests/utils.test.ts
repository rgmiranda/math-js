...
import { range, Vector } from "../src";
import { getOrientation } from "../src/utils";
...
describe(getOrientation.name, () => {
  ...
  it.each(testData)("calculates orientation", ({ p, q, r, expected }) => {
    expect(getOrientation(p, q, r)).toBe(expected);
  });
});
